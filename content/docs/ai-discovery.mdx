...
| `/docs/<slug>.md` and content endpoints  | Page-level markdown representation                             | `src/proxy.ts`, `src/app/[locale]/llms.mdx/docs/[[...slug]]/route.ts`                                         |
| `/blog/<slug>.md` | Blog text with author and publication date | `src/lib/blog-source.ts`, `src/app/[locale]/llms.mdx/blog/[slug]/content.md/route.ts` |
| `/templates/<slug>.md` | Template block recipe, install command, and page plan | `src/lib/template-markdown.ts`, `src/app/[locale]/llms.mdx/templates/[slug]/content.md/route.ts` |
...
| canonical, Open Graph, and RSS metadata  | Browser/search identity and sharing declarations               | `src/app/[locale]/layout.tsx`, route metadata                                                                 |
...
Docs, blog posts, and template detail pages accept `Accept: text/markdown` as well
as the `.md` suffix. Their markdown responses carry `X-Robots-Tag: noindex` so the
HTML page stays the search target. Locale-prefixed URLs follow the same content
fallback as HTML. Template markdown includes the installation boundary: the CLI
installs the block set; it does not seed the concept's curated page content.
...
- Docs dates come from Git history. Marketing and template pages omit the field.
