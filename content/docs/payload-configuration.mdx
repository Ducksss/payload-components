...
`cross-env` is what makes that assignment portable: a bare `PAYLOAD_CONFIG_PATH=... payload` prefix is POSIX shell syntax, so it fails in `cmd.exe` and PowerShell. It is not part of Payload, so install it in the project first:

<RunnableCommand
  command="pnpm add -D cross-env"
  label="Copy the cross-env install command"
/>

On a project that never runs these scripts on Windows, drop the prefix and keep the bare `PAYLOAD_CONFIG_PATH=src/payload.config.ts payload generate:types`.
