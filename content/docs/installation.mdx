...
<Steps>
  ...
  <Step>**Build the registry** from the checked-in manifest, then **add the registry item** and install its source files.</Step>
  <Step>
    **Install missing component dependencies** so the block compiles in your project.
  </Step>
  <Step>**Apply Payload fragments** — register the block in `RenderBlocks` and the `Pages` layout field.</Step>
  <Step>
