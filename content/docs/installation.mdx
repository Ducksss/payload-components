...
- private ownership: a random token plus the exact Page and Media IDs
...
existing same-slug Page is always a collision—matching public title or marker
text is not enough. After creation, the script saves the returned Page ID. A
rerun updates only that exact ID after verifying the expected slug, title,
block type, and tokenized marker; the update also uses `overrideLock: false`.
Missing or mismatched local state causes a hard failure before unsafe mutation.

Components with required uploads get one ownership-marked placeholder Media
document, immediately saves its returned ID, and reuses only that exact ID on a
rerun. If the Page write fails, the saved Media ID makes a retry idempotent.
