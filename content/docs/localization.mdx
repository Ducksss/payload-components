...
A whole template takes the same flag, so a fifteen-block site does not need a second pass:
...
`localize` does not write this — which admin languages to ship is your call, and the import
is a one-liner. Check `@payloadcms/translations/languages/` for the codes Payload ships;
a content locale with no admin translation is fine, the chrome just stays in the fallback
language.
...
<Callout type="warn">
  ...
  land under the default locale, and the change is not free to undo. Back up and migrate a
  database that already holds content before running this, and run `--dry-run` first.
</Callout>
