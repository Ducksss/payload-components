...
only validate the installed files and fragments and atomically write
`payload-components/seed-<component>.ts`. The CLI does not import Payload, open a
database, or make a network request during seed generation.
...
requires Pages drafts, creates or updates a draft with an exact ownership
marker, and limits placeholder-media reuse and cleanup to the same exact marker.
This keeps elevated `overrideAccess` calls visible in operator-run target code
instead of hiding them inside the distribution CLI.
