...
A file that differs from what ships now is reported as `outdated` when it still matches what
the install wrote, and `modified` only when it matches neither. Those need opposite responses
— an outdated file wants an ordinary `update`, a modified one wants a decision about whose
version to keep — so they are never collapsed into one label.
...
**A file left behind by an older version is not a local edit.** When a version bump rewrites
a file, the copy on disk stops matching what ships — that is the upgrade, not drift, and
`update` replaces it without asking. Install state records a hash of every file as it was
written, which is what separates the two; an install recorded before this CLI tracked those
hashes cannot make the distinction, so it still reports the file as modified and says so in
the skip message.
