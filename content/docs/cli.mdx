...
npx payload-components update --accept-localization-policy-change
```

Re-installs recorded components at the version this CLI ships. Canonical source replacements
...
These are separate decisions with separate flags: `--force` means "discard my local
...
npx payload-components remove hero-basic --force --accept-stored-content
```
...
- **Stored content is untouched.** Existing Page documents keep their block data. A real
  removal therefore stops until you pass `--accept-stored-content`, confirming that you
  migrated or deleted those blocks in the admin. This consent is independent from `--force`,
  which only accepts source-ownership risk.
...
This is repository-only contributor tooling and is not part of the published consumer CLI.
It scaffolds a whole component bundle in a checkout of this repository: the block source,
manifest, registry item, doc page, and demo twin, plus entries in the demos registry, the
docs sidebar, generated site-catalog projection, and README inventory table. Consumer CLI
help derives its component list from the registry at runtime.
...
<Callout type="info">
  ...
  Pages collection keeps it as an unowned file the scaffold will not touch, and a rerun creates
  nothing new. What a rerun does touch is what it owns: managed files still pristine are
  updated, and managed files you have edited are reported rather than replaced unless you pass
