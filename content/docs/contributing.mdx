...
<Steps>
  ...
  <Step>
    **Installable source stays portable.** Do not use site-only utilities such as
    `rounded-frame` or `tracking-display` in `payload-components/source`; consumer
    projects do not get this site's `globals.css`. Use normal Tailwind utilities,
    including arbitrary radius/tracking values when the design needs them.
  </Step>
</Steps>

`tests/int/visual-standards.int.spec.ts` enforces this statically across `payload-components/source` (and holds the landing twins to the colour palette), while `tests/e2e/components-visual.e2e.spec.ts` snapshots every component to catch anything pixels can see. Site-only tokens are fine in site chrome, but copied Payload block source must stand alone in consumer projects.

<Callout type="info">
  Pure layout geometry — `aspect-[4/3]`, `grid-cols-[auto_1fr_auto]`,
  `w-[calc(…)]` — is per-component and stays arbitrary by design; colours stay
  token-governed.
</Callout>
