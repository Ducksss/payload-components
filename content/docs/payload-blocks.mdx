---
title: "Payload CMS blocks: create, register, type, and render in v3"
seoTitle: "Payload CMS blocks: create, register, type, and render"
...
Payload CMS blocks in v3 become live through one chain: define the `Block` config, register it in a collection, generate its type, map its slug to a frontend component, refresh the admin import map, then add the block to a page. Miss one handoff and the block is absent from the editor, untyped, or invisible on the frontend.
...
**Want to inspect the plan first?** The dry run performs the same project and
dependency validation, then reports the files, wiring changes, packages,
generation commands, and install-state update it would make. It does not write
files, install packages, run commands, or update install state.

<RunnableCommand
  command="npx payload-components add hero-basic --dry-run"
  label="Copy the hero-basic dry-run command"
/>

Run the install on a branch, inspect the resulting diff, then open `/admin` and
publish a test Page. The [installation guide](/docs/installation) lists the
supported project shape and the read-only `doctor` check.
