...
<BlogFigure
  ...
  alt="A CLI contract diagram separating manifest files, dependencies, source fragments, and post-install tasks from the install state that records their outcome"
  caption="The manifest declares work; the separate state record records outcomes after an attempt, failure, or successful install."
/>
...
`lastError`, and `patchedFiles`. It records results rather than telling a retry where to begin. It is
not allowed to overrule the filesystem. If the manifest expects an owned file and a developer
deleted it, `doctor` reports the mismatch.
