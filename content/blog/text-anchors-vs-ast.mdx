...
<BlogFigure
  ...
  alt="A before-and-after source diagram where a renderer entry is inserted at one verified text anchor without changing surrounding code"
  caption="The patch is intentionally smaller than the parser problem: verify one stable anchor, deduplicate one insertion, and preserve everything else byte for byte."
/>
...
Project detection runs before mutation. Fragment application verifies each anchor and checks for
ambiguity. A missing anchor produces an actionable failure with the affected stage. The consumer can
integrate manually, adapt the repository, or contribute a tested supported shape.
...
## Preflight reduces partial source mutation

Before applying fragments, the installer resolves target files and validates the expected anchors.
A robust patch sequence should discover obvious incompatibility before writing the first host file.
Once execution begins, each completed stage is recorded so an unexpected later failure remains
diagnosable.

Atomic replacement of an individual file prevents a process interruption from leaving half an
insertion. It does not make a multi-file installation globally atomic. That is why install state and
idempotent recovery exist. The [idempotency guide](/blog/idempotent-code-installer) covers the larger
state machine.
