...
| TypeScript reports stale or missing block fields | `src/payload-types.ts` was generated before the block was registered | Run `pnpm payload generate:types`. |
| The admin UI cannot load the block, or reports an import-map error | The generated admin import map is stale | Run `pnpm payload generate:importmap`. Do not edit the generated map by hand. |
...
<Callout type="info">
  Already installed with Payload Components? Run `npx payload-components doctor` before editing.
