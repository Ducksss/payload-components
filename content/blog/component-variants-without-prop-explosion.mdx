...
cover:
  ...
  alt: Feature catalog results beside a comparison between an overloaded prop matrix and the explicit Bento, Split, Steps, and Grid Basic variants.
tags:
  - Component design
  - Payload CMS
  - TypeScript
...
needs reversed media, another needs video, another needs a different content hierarchy, and the
component accumulates switches until no one knows which combinations are valid.
...
<BlogFigure
  ...
  alt="A comparison between one overloaded component with a matrix of prop combinations and a family of three focused structural variants"
  caption="A family makes valid structures explicit. An overloaded prop matrix makes every combination look supported, including the ones nobody designed or tested."
/>
...
Examples include a split hero versus a full-bleed video hero, a feature grid versus a sequential
steps layout, or pricing cards versus an enterprise comparison.
...
A family member always carries a structural suffix: `feature-bento`, `feature-split`,
`feature-steps`. Do not ship a bare `feature` item and later discover that it actually meant one
arbitrary layout.
...
express rules such as “video is required only when variant is cinematic, unless compact is true.”
The Payload config and generated interface describe one valid structure.
...
```ts
export const featureIntroFields: Field[] = [
