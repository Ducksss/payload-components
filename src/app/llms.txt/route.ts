...
export function GET() {
  const body = [
    ...
    `- [shadcn vs Payload Components](${siteUrl}/compare/shadcn-vs-payload-components)`,
    `- [About](${siteUrl}/about)`,
    ...
    '',
    '## shadcn comparison',
    'For a Payload Components layout item, raw shadcn registry delivery copies the declared source files.',
    'The Payload Components wrapper uses the same registry item, then registers the block in Pages,',
    'maps its renderer, regenerates Payload types and the admin import map, and records install state.',
    '',
    '## Supported stack',
