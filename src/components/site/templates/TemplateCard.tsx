...
export function TemplateCard({
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-frame border border-border bg-card shadow-card transition-shadow duration-300 hover:shadow-frame">
      <TemplateCardPoster
        alt={t('posterAlt', { title: template.title })}
        height={TEMPLATE_POSTER_HEIGHT}
        href={detailHref}
        label={t('posterLabel', { title: template.title })}
        priority={priority}
      ...
      <div className="flex flex-1 flex-col gap-4 p-6 sm:p-7">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ul className="flex flex-wrap items-center gap-1.5" aria-label={t('visualTone')}>
          ...
          <p className="ml-auto shrink-0 font-mono text-xs text-muted-foreground">
            {t('counts', { blocks: blockCount, pages: template.pages.length })}
          </p>
        </div>
