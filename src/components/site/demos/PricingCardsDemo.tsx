...
import { pricingCardsDemoContent, type PricingDemoContent } from '@/lib/demo-content'
import { cn } from '@/utilities/ui'

import { DemoLink } from './DemoLink'

/* DEMO TWIN of payload-components/source/blocks/PricingCards/Component.tsx
 * (pricing-cards@0.1.0). Class strings are copied verbatim from the component
 ...
 *   PricingCardsBlockData                    → PricingDemoContent (@/payload-types is consumer-only)
...
export function PricingCardsDemo({
  className,
  content = pricingCardsDemoContent,
}: {
  className?: string
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {plans.map((plan, index) => (
              <Card
                key={`${plan.name}-${index}`}
                className={cn(
                  'relative flex flex-col border-border/70 bg-background/85 shadow-none',
                  {
