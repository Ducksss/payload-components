...
import { testimonialsRatingDemoContent, type TestimonialDemoContent } from '@/lib/demo-content'

/* DEMO TWIN of payload-components/source/blocks/TestimonialsRating/Component.tsx
 * (testimonials-rating@0.1.0). Class strings are copied verbatim from the
 ...
 *   TestimonialsRatingBlockData              → TestimonialDemoContent (@/payload-types is consumer-only)
 *   cn() outer wrapper + star fill           → plain class strings (skipped by the class-mirror guard)
 * imgClassName values are not mirrored. If the component Component.tsx changes,
 * update this file in the same PR. */

export function TestimonialsRatingDemo({
  className,
                      ...
                      <Star
                        ...
                        className={
                          starIndex < (item.rating ?? 0)
                            ? 'size-4 fill-primary stroke-primary'
                            : 'size-4 fill-muted stroke-muted-foreground/30'
                        }
                        key={starIndex}
                      />
                    ))}
