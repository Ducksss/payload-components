...
import { pricingCardsMutedDemoContent, type PricingDemoContent } from '@/lib/demo-content'
import { cn } from '@/utilities/ui'

import { DemoLink } from './DemoLink'

/* DEMO TWIN of payload-components/source/blocks/PricingCardsMuted/Component.tsx
 * (pricing-cards-muted@0.1.0). Class strings are copied verbatim from the
 ...
 *   PricingCardsMutedBlockData               → PricingDemoContent (@/payload-types is consumer-only)
...
export function PricingCardsMutedDemo({
  className,
  content = pricingCardsMutedDemoContent,
}: {
  className?: string
        ...
        <div className="mx-auto flex max-w-6xl flex-col gap-10">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {plans.map((plan, index) => (
              <Card
                key={`${plan.name}-${index}`}
                className={cn('relative flex flex-col border-transparent bg-muted shadow-none', {
