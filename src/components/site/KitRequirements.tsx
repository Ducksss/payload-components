...
import { getKitManifest, getKitRegistryDependencies, getSupportTarget } from '@/lib/kit-manifest'

/* Backs <KitRequirements slug="…" /> — honest, manifest-driven compatibility:
 * supported target, Payload/Next majors, and the shadcn UI it pulls in. */
export async function KitRequirements({ slug }: { slug: string }) {
  const manifest = await getKitManifest(slug)

  if (!manifest) return null

  const [deps, target] = await Promise.all([
    getKitRegistryDependencies(slug),
    ...
    <div className="not-prose my-6 flex flex-col gap-4">
      ...
      {target?.requiredFiles.length ? (
        <p className="text-sm leading-6 text-muted-foreground">
          ...
          — the surfaces <code className="font-mono text-[13px]">payload-kit add</code> patches. The
