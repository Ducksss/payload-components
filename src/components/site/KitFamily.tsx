...
/* Backs <KitFamily slug="…" /> — the sibling variants in this kit's family
 * (same taxonomy as the sidebar grouping), so readers can pick the right layout.
 * Renders nothing for a family of one. */
export function KitFamily({ slug }: { slug: string }) {
  const familyKey = familyOfSlug(slug).key
  const siblings = kitEntries.filter((kit) => familyOfSlug(kit.slug).key === familyKey)
