      ...
      <TabsList className="h-auto w-full flex-wrap justify-start gap-2 rounded-[1.5rem] bg-transparent p-0">
        {kitCategories.map((category) => (
          <TabsTrigger
            ...
            className="rounded-full border border-border/80 bg-background px-4 py-2.5 data-[state=active]:border-foreground/10 data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-none"
          >
            {category.label}
          </TabsTrigger>
        ))}
      ...
      {kitCategories.map((category) => (
        <TabsContent key={category.value} value={category.value} className="mt-0">
          <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
            <div className="rounded-[1.75rem] border border-border/70 bg-card/60 p-6 backdrop-blur-sm">
              <Badge
                variant="outline"
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {category.kits.map((kit) => (
                <Card
                  ...
                  className={cn(
                    styles.panelLift,
                    'rounded-[1.5rem] border-border/70 bg-background/85 shadow-none',
                  )}
                >
