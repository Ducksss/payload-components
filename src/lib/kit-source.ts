 ...
 * and any shared `*Fields.ts`. Resolved through `payload-kits/registry.json` (the
...
export type KitSourceFile = { code: string; lang: string; title: string }
...
export async function getKitSources(slug: string): Promise<KitSourceFile[]> {
  ...
  const sources = await Promise.all(
    [...files]
      ...
      .map(async (file): Promise<KitSourceFile | null> => {
        try {
  ...
  return sources.filter((source): source is KitSourceFile => source !== null)
}
