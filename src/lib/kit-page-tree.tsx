...
import { Blocks, FileText, LayoutGrid, LayoutPanelTop } from 'lucide-react'
 ...
 *
 * The kit doc pages live flat in `content/docs/kits/` (URLs stay `/docs/kits/<slug>`),
 ...
 * filesystem, meta.json, URLs, or `kitEntries`, so search/sitemap/structured-data and
 * every `/docs/kits/<slug>` link are unaffected. Registered via
...
/* Install modes, in display order. `key` matches `kitEntries[].family`. */
...
const modeOfSlug = (slug: string): InstallMode => {
  ...
  const family: string | undefined = kitEntries.find((kit) => kit.slug === slug)?.family
...
/* The kits folder is the only top-level folder whose pages live under /docs/kits/. */
const isKitsFolder = (node: TreeNode): node is Folder =>
  node.type === 'folder' &&
  node.children.some((child) => child.type === 'page' && child.url.startsWith('/docs/kits/'))
...
export function regroupKitTree(root: Root): Root {
