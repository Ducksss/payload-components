...
describe('Fumadocs site shell', () => {
  ...
  it('keeps catalog search local and docs copy factual', async () => {
    ...
    expect(catalogTitle).toBe('71 Payload CMS components and typed blocks')
    expect(catalogDescription).toMatch(
      /heroes.*features.*pricing.*integrations.*FAQs.*content.*teams.*embeds/,
    )
    expect(catalogDescription).toContain('Browse all 71')
    ...
    expect(catalogMetadataTitle).toContain('71')
  ...
  it('gives nearby search surfaces distinct jobs and routes catalog intent to components', async () => {
    ...
    expect(blogTitle).toBe('Payload CMS block and installer guides')
    expect(catalogMetadataTitle).toBe('71 Payload CMS Components & Blocks | Catalog')
