...
import {
  PAGES_LAYOUT_FILE,
  RENDER_BLOCKS_FILE,
} from "../../tools/payload-components/constants";
import { loadManifest } from "../../tools/payload-components/manifest";

import {
  expectInstalledComponents,
  readInstallState,
} from "./payload-components-assertions";
import {
...
const payloadComponentBin = path.join(
  repoRoot,
  "bin",
  "payload-components.mjs",
);
...
const runAddCommand = async (fixtureDir: string, componentName: string) =>
  execFileAsync(
    process.execPath,
    [payloadComponentBin, "add", componentName, "--cwd", fixtureDir],
    {
      cwd: repoRoot,
      env: process.env,
  ...
  it("keeps every manifest wired to registry source, docs, and recovery targets", async () => {
    ...
    for (const name of names) {
      ...
      for (const file of registryItem?.files ?? []) {
        await expect(
          readFile(path.join(repoRoot, file.path), "utf8"),
        ).resolves.toBeTruthy();
      }

      await expect(
        readFile(
          path.join(repoRoot, "content", "docs", "components", `${name}.mdx`),
          "utf8",
        ),
      ).resolves.toContain(`npx payload-components add ${name}`);

      for (const fragment of manifest.payloadFragments) {
        expect(manifest.recovery.patchedFiles).toContain(
          expectedPatchFor(fragment.kind),
        );
      }
    }
  });
...
describe("payload-components add", () => {
  ...
  it.each(representativeInstallComponents)(
    ...
    async (componentName) => {
      const { fixtureDir, manifest } =
        await createInstallFixture(componentName);
  ...
  it("installs multiple components without duplicate registrations", async () => {
    ...
    const { fixtureDir, manifests } =
      await createInstallFixtureForComponents(componentNames);
    tempDirs.push(fixtureDir);
