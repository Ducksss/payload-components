...
async function getGtagEvents(page: Page) {
  return page.evaluate(
    () =>
      (window as Window & { __gtagEvents?: unknown[][] }).__gtagEvents ?? [],
  );
}

async function expectCopiedAlert(page: Page) {
  await expect(
    page.getByRole("alert").filter({ hasText: copiedAlertText }),
  ).toBeVisible({
    timeout: 15000,
  });
}
...
test.describe("Light shadcn frontend", () => {
  ...
  test("renders the light token-driven homepage", async ({ page }) => {
    ...
    await expect(
      page.locator("code", { hasText: primaryInstallCommand }).first(),
    ).toBeVisible();
  ...
  test("aligns the brand mark relative to the documentation rail", async ({
    page,
  }) => {
    const viewports = [
      { alignTo: "header-padding", height: 844, name: "mobile", width: 390 },
      {
        alignTo: "documentation-title",
        height: 1024,
        name: "tablet",
        width: 768,
      },
    ...
    for (const viewport of viewports) {
      ...
      const geometry = await page.evaluate(() => {
        ...
        const documentationTitle = [
          ...document.querySelectorAll("#nd-sidebar span"),
        ].find((element) => element.textContent?.trim() === "Documentation");
        const brandMarkRect = brandMark?.getBoundingClientRect();
        const headerInnerRect = headerInner?.getBoundingClientRect();
        const documentationTitleRect =
          documentationTitle?.getBoundingClientRect();
        ...
        return {
          ...
          documentationTitle:
            documentationTitleRect && documentationTitleRect.width > 0
              ? {
                  width: documentationTitleRect.width,
                  x: documentationTitleRect.x,
                }
              : null,
      ...
      if (viewport.alignTo === "documentation-title") {
        ...
        expect(
          Math.abs(
            geometry.brandMark!.x -
              (geometry.headerStart! + geometry.headerPaddingStart!),
          ),
          viewport.name,
        ).toBeLessThanOrEqual(1);
      }
  ...
  test("exposes docs, catalog, component pages, and no horizontal overflow", async ({
    page,
  }) => {
    const routes = [
      {
  ...
  test("serves the standalone preview route without site chrome or overflow", async ({
    page,
  }) => {
  ...
  test("marks only the current top-level navigation item active", async ({
    page,
  }) => {
    await page.goto(baseURL);
    await expect(
      page.getByRole("navigation").locator("a.bg-secondary"),
    ).toHaveCount(0);
  ...
  test("redirects old kit docs URLs to component docs", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "Hero Basic" }),
    ).toBeVisible();
  ...
  test("exposes every landing section, the catalog teaser, and the footer", async ({
    page,
  }) => {
    await page.goto(baseURL);
    ...
    await expect(
      page.getByRole("heading", { name: "Page blocks" }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Browse all \d+ components/ }),
    ).toBeVisible();
    await expect(
      page.locator("code", { hasText: primaryInstallCommand }).first(),
    ).toBeVisible();
  ...
  test("exposes the Fumadocs docs shell navigation", async ({ page }) => {
    ...
    await expect(
      sidebar.getByRole("link", { name: "Architecture" }),
    ).toBeVisible();
    await expect(
      sidebar.getByRole("link", { name: "Installation" }),
    ).toBeVisible();
    await expect(
      sidebar.getByRole("link", { name: "Registry Contract" }),
    ).toBeVisible();
    // Components are grouped install-mode → family in the sidebar (see src/lib/component-page-tree).
    await expect(
      sidebar.getByRole("button", { name: "Page blocks" }),
    ).toBeVisible();
    await expect(
      sidebar.getByRole("button", { name: "Feature" }),
    ).toBeVisible();
  ...
  test("copies a catalog family-card command", async ({ page, context }) => {
    ...
    const catalogComponent = componentEntries.find(
      (entry) => entry.slug === "feature-bento",
    )!;

    await context.grantPermissions(["clipboard-read", "clipboard-write"]);
    await page.goto(baseURL);
    await page
      .locator(`#${catalogComponent.slug}`)
    ...
    await page.evaluate(() => {
      document.addEventListener(
        ...
        (event) => {
          ...
          if (
            target.closest(
              'a[href="https://github.com/Ducksss/payload-components"]',
            )
          ) {
  ...
  test("shows an alert after copying a docs code block", async ({
    page,
    context,
  }) => {
    await context.grantPermissions(["clipboard-read", "clipboard-write"]);
  ...
  test("shows an alert after copying page markdown", async ({
    page,
    context,
  }) => {
    await context.grantPermissions(["clipboard-read", "clipboard-write"]);
...
test.describe("Reduced motion", () => {
  ...
  test("landing page keeps its desktop and mobile visual contract", async ({
    page,
  }) => {
    await page.goto(baseURL);
    await expect(
  ...
  test("terminal replay renders its final transcript without animation", async ({
    page,
  }) => {
    await page.goto(baseURL);
  ...
  test("opens the Fumadocs search dialog from the docs shell", async ({
    page,
  }) => {
    await page.goto(`${baseURL}/docs`);
