...
| `tools/payload-components/` + `bin/payload-components.mjs` | The CLI: `add`, `add-template`, `templates`, `list`, `diff`, `update`, `remove`, `seed`, `init`, `doctor`, `mcp`; project detection, fragment patching, install state, registry build/check                        |
