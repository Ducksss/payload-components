...
- **`tests/int/`** — demo-twin class-mirror fidelity; **component visual standards** (`visual-standards.int.spec.ts`: token-only colours and no site-only radius/tracking utilities in installable source); registry reproducible; install idempotent and recoverable.
