...
_Config level_ — `localize [component...] [--locales en,zh] [--default-locale <code>] [--no-fallback]` (`commands/localize.ts`) inserts `localization: { defaultLocale, fallback, locales }` into the consumer's `buildConfig({ ... })` call and applies the field-level wrap to every recorded install (or the named ones). The locale table lives in `tools/payload-components/locales.ts` — native labels (`zh` → `简体中文`) and `rtl` for the scripts that need it; codes are shape-validated because they are written into generated TypeScript. `setPayloadLocalization` / `readPayloadLocalization` in `project.ts` do the anchoring with the same masking scanner as the fragment patcher, and refuse rather than rewrite three shapes: no `buildConfig({` call, a `localization` value that is not an object literal, and an existing block declaring something else (`--force` replaces that one). Re-running is a clean no-op.

Two bookkeeping rules make the after-the-fact wrap safe. `recordLocalizedInstall` (`state.ts`) re-snapshots only the files the run rewrote, so `diff` does not read the wrapper as consumer drift and `update` keeps it; and a block config with local edits is skipped unless `--force`, because re-baselining it would bless a consumer edit as pristine. `doctor` reports both halves and names whichever is missing; `add --localized` prints the `localize` command when the config declares no locales.
...
- **`tests/e2e/components-visual.e2e.spec.ts`** — each demo twin matches its committed Linux baseline at `/components/preview/<slug>`. Mint intentional changes using the `visual-baselines` workflow and review its PNG PR. Linux CI fails if any baseline is missing. Other platforms skip screenshot comparisons while retaining browser behavior and accessibility checks.
...
`pnpm test:release` runs the full local gate. `pnpm test:fresh` is the external Payload smoke test for consumer-affecting PRs, production pushes and nightly confidence (one bare job, four website shards; site-only PRs skip consumer jobs); it runs two scenarios and `--scenario bare|website|all` selects between them. The **bare** scenario compiles the starter base inside a real Payload app — this repo excludes `payload-components/source` from tsc and has no `payload` dependency — so run it after touching `source/base` or the fragment patcher. It caught three shipped bugs the moment it first ran.
