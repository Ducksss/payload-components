...
import type { ContentRowsBlock as ContentRowsBlockData } from '@/payload-types'

import { Media } from '@/components/Media'
import { Badge } from '@/components/ui/badge'
import { cn } from '@/utilities/ui'

type Props = ContentRowsBlockData & {
  id?: string
  className?: string
  disableInnerContainer?: boolean
}

export const ContentRowsBlock: React.FC<Props> = ({
  className,
        ...
        >
          ...
          {rows && rows.length > 0 ? (
            <div className="flex flex-col gap-12">
              {rows.map((row, index) => (
                <div
                  className="grid gap-6 sm:grid-cols-2 sm:items-center sm:gap-12"
