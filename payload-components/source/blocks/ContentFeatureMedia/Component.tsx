...
import type { ContentFeatureMediaBlock as ContentFeatureMediaBlockData } from '@/payload-types'
...
type Props = ContentFeatureMediaBlockData & {
  id?: string
  className?: string
  disableInnerContainer?: boolean
}

export const ContentFeatureMediaBlock: React.FC<Props> = ({
  className,
        ...
        >
                  ...
                  return (
                    <div className="flex flex-col gap-2" key={feature.id ?? `${feature.title}-${index}`}>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-sm font-medium tracking-[-0.01em]">{feature.title}</h3>
                      </div>
          ...
          {image ? (
            <div className="overflow-hidden rounded-[1.5rem] border border-dotted border-border/70 p-2">
              <Media resource={image} imgClassName="h-full w-full rounded-[1rem] object-cover" />
            </div>
          ) : null}
        </div>
      </div>
