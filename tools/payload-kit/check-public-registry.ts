...
export const buildRegistryForCheck = async () => {
  const outputDir = await mkdtemp(path.join(os.tmpdir(), 'payload-kit-registry-check-'))

  await runCommand({
    args: ['dlx', shadcnCliPackage, 'build', 'payload-kits/registry.json', '--output', outputDir, '--cwd', '.'],
    command: 'pnpm',
    cwd: repoRoot,
  })
...
export const assertGeneratedRegistryMatchesSource = async (publicRegistryDir: string) => {
  ...
  assertEqual(publicRegistry, sourceRegistry, 'generated registry.json is out of date with payload-kits/registry.json.')
