...
const runCommand = async ({
  ...
  new Promise<void>((resolve, reject) => {
    console.log(`\n[payload-kit smoke] ${stage}`)
    console.log(`[payload-kit smoke] $ ${command} ${args.join(' ')}`)
    console.log(`[payload-kit smoke] cwd=${cwd}`)
...
const runFreshPayloadRepoSmoke = async ({
  ...
  const projectName = 'payload-kit-smoke-target'
  ...
  await runCommand({
    ...
    command: 'pnpm',
    cwd: targetPath,
    stage: 'install local payload-kit package into fresh project',
    timeoutMs,
  })

  for (const kit of kits) {
    await runCommand({
      args: ['exec', 'payload-kit', 'add', kit],
      command: 'pnpm',
      cwd: targetPath,
      stage: `payload-kit add ${kit} in fresh project`,
      timeoutMs,
    })
  }
  ...
  await runCommand({
    args: ['exec', 'tsx', '.payload-kit/smoke-seed.ts'],
    command: 'pnpm',
    cwd: targetPath,
...
const smokeEnvForTarget = (serverUrl: string): Partial<NodeJS.ProcessEnv> => ({
  CRON_SECRET: process.env.CRON_SECRET ?? 'payload-kit-smoke-cron-secret',
  NEXT_PUBLIC_SERVER_URL: serverUrl,
  PAYLOAD_SECRET: process.env.PAYLOAD_SECRET ?? 'payload-kit-smoke-secret',
  PREVIEW_SECRET: process.env.PREVIEW_SECRET ?? 'payload-kit-smoke-preview-secret',
})
...
const writeSummary = async (summary: SmokeSummary) => {
  ...
  console.log('\n[payload-kit smoke] summary')
  console.log(JSON.stringify(summary, null, 2))
}

export const runSmoke = async (options: SmokeOptions) => {
  const tempRoot = await mkdtemp(path.join(tmpdir(), 'payload-kit-fresh-smoke-'))
  ...
  try {
    ...
    if (success && !options.keepTemp) {
      ...
      console.log(`[payload-kit smoke] temp preserved at ${tempRoot}`)
    }
  }
}
