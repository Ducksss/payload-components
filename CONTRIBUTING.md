...
| Site UI (`src/app`, `src/components`, `src/lib`)       | `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm run test:e2e` |
| Registry or component metadata (`payload-components/`) | `pnpm test:registry`, `pnpm run test:int`                  |
| CLI or tooling (`tools/payload-components`, `bin/`)    | `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm test:install` |
